...
@section('content')
    ...
    <div class="max-w-2xl">
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 mt-4">
            <h2 class="text-base font-semibold text-white mb-6">Edit Showtime</h2>

            <form method="POST" action="{{ route('admin.showtimes.update', $showtime->showtime_id) }}" class="space-y-5">
                @csrf @method('PUT')
                    ...
                    <select name="hall_id" required
                        ...
                        @foreach($halls as $hall)
                            <option value="{{ $hall->hall_id }}"
                                {{ old('hall_id', $showtime->hall_id) == $hall->hall_id ? 'selected' : '' }}>
