...
@section('content')
    ...
    <div class="max-w-2xl">
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 mt-4">
            <h2 class="text-base font-semibold text-white mb-6">Edit Movie</h2>

            <form method="POST" action="{{ route('admin.movies.update', $movie->movie_id) }}" class="space-y-5">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                        class="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2.5
                               text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition resize-none
                               @error('description') border-red-500 @enderror">{{ old('description', $movie->description) }}</textarea>
                    ...
                    <select name="genre_id"
                        ...
                        @foreach($genres as $genre)
                            <option value="{{ $genre->genre_id }}"
                                {{ old('genre_id', $movie->genre_id) == $genre->genre_id ? 'selected' : '' }}>
                                {{ $genre->genre_name }}
                            </option>
                        @endforeach
                    </select>
