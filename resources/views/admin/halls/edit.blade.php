...
@section('content')
    ...
    <div class="max-w-2xl">
        ...
        <div class="bg-gray-900 rounded-xl border border-gray-800 p-6 mt-4">
            <h2 class="text-base font-semibold text-white mb-1">Edit Hall</h2>
            <p class="text-sm text-gray-400 mb-6">{{ $hall->cinema->name }}</p>

            <form method="POST" action="{{ route('admin.halls.update', $hall->hall_id) }}" class="space-y-5">
                @csrf @method('PUT')

                <div>
                    <label class="block text-sm text-gray-300 mb-1.5">Hall Name</label>
                    <input type="text" name="hall_name" value="{{ old('hall_name', $hall->hall_name) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="capacity" value="{{ old('capacity', $hall->capacity) }}" min="1" required
                        class="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2.5
                               text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition
